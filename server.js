...
function normalise(notices, source, country) {
  return notices
    ...
    .slice(0, 30)
    .map(n => ({ ...n, source, isLive: true, country: n.country || country || '' }));
}
...
async function fetchDoffinBelowThreshold(keywords) {
  const terms = (keywords || DEFAULT_KEYWORDS).filter(k => k.split(' ').length <= 3).slice(0, 4);
  const raw = await playwrightScrape({
    ...
    extractFromDom: () => {
      ...
      return cards.slice(0,15).map(el => {
        ...
        return { id, title: title.trim(), org: org.trim(), country: 'NO', deadline: 'unknown', published: 'unknown', value: 'N/A', url: href, keywords: [], isNew: false };
      }).filter(Boolean);
    }
  });
  return normalise(raw, 'doffin', 'NO');
}
  ...
  const raw = await playwrightScrape({
    ...
    extractFromDom: () => {
      ...
      return cards.slice(0,15).map(el => {
        ...
        const id = el.dataset.noticeId || href.split('/').pop() || Math.random().toString(36).slice(2,10);
        if (!title.trim()) return null;
        return { id, title: title.trim(), org: org.trim(), country: 'DK', deadline: 'unknown', published: 'unknown', value: 'N/A', url: href, keywords: [], isNew: false };
      }).filter(Boolean);
    }
  });
  return normalise(raw, 'udbud', 'DK');
}
...
async function fetchFindATender() {
  ...
  return (data.releases || []).slice(0,25).map(r => {
    ...
    return {
      ...
      keywords: [], isNew: false, source: 'fat', isLive: true
    };
  }).filter(n => n.title);
}
